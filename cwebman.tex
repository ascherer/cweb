...
optional. Include files can nest. Change files cannot use the \.{@i} feature.

\@( [C,\oT] A module name can begin with \.{@(} instead of \.{@<}.
Everything works exactly as before, except that \.{@(foo@>} denotes
a special module name all of whose \Cee\ code is written by \.{CTANGLE}
...
have large \.{WEB} files or modules, you may need to see
...
been somewhat mystified, since it thinks that module names
...
to Appendix~A.  Notice that modules~13, 14, 15,
and~16 have been tangled into module~12.
...
\X15:Skip to the next nonblank line; \&{return} if end of file\X;\6
\X16:Move \\{buffer} and \\{limit} to \\{change\_buffer} and \\{change\_limit}%
\X;\6
\4${}\}{}$\2\par
\fi
...
\M14. We do not allow includes in a change file, so as to avoid confusion.

\Y\B\4\X14:Check for erroneous \.{@i}\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\\{buffer}[\T{1}]\E\.{'i'}){}$\5
${}\{{}$\1\6
${}\\{loc}\K\\{buffer}+\T{2};{}$\6
\\{err\_print}(\.{"!!\ No\ includes\ allow}\)\.{ed\ in\ change\ file"});\6
\4${}\}{}$\2\6
\4${}\}{}$\2\par
\Us13\ET25.\fi
!endgroup
...
for every starred module. The file \.{common.toc} might look like this:
