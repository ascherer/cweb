...
\xdef\fmtversion{\fmtversion+CWEB3.6}
\newif\ifpdf
\ifx\pdf+\pdftrue\fi
...
\def\stsec{\rightskip=0pt % get out of C mode (cf. \B)
  ...
  \ifpdf{\raise\baselineskip\hbox to0pt{%
     \let\*=\empty\special{%
       pdf: dest (\romannumeral\secstar) [ @thispage /FitH @ypos ]}}}\fi}
...
\def\inx{\par\vskip6pt plus 1fil % we are beginning the index
  ...
  \outer\def\I##1, ##2.{\par\hangindent2em\noindent##1:\kern1em
    \ifpdf\pdfnote##2.\else##2\fi.} % index entry
...
\def\fin{\par\vfill\eject % this is done when we are ending the index
  ...
  \ifpdf
    ...
    \let\Xpdf\X
    \def\X##1:##2\X{\Xpdf##1:##2\X \toks0{##2} \special{pdf: outline 0
      << /Title (\the\toks0) /A << /S /GoTo /D (\romannumeral##1) >> >>}}
  \fi
  \let\topsecno=\nullsec
