...
% Version 4.6.1 --- January 2022
\ifx\renewenvironment\undefined\else\endinput\fi % LaTeX will use other macros
\xdef\fmtversion{\fmtversion+CWEB4.6.1}
\chardef\cwebversion=4 \chardef\cwebrevision=6 \chardef\cwebsubrevision=1
\newif\ifpdf
\ifx\pdf+\pdftrue\fi
...
\fi\fi
\newif\ifHINT
\expandafter\ifx\csname HINTversion\endcsname\relax\HINTfalse
\else\HINTtrue\fi
...
\def\note#1#2.{\Y\noindent{\hangindent2em%
    \baselineskip10pt\eightrm#1~\ifacro{\pdfnote#2.}\else
    \ifHINT{\HINTnote#2.}\else#2\fi\fi.\par}}
...
\def\HINThome{\HINTdest name {HINT.home}%
  \HINToutline goto name {HINT.home} depth 1 {Table of Content}}
...
% The following is a copy of \pdfnote with minimal changes
% to have \makenote use \HINTlink instead of \pdflink
% The duplicated code should probably be merged.
\def\HINTnote#1.{\setbox0=\hbox{\toksA={#1.}\toksB={}\maketoks}\the\toksA}
\def\firstsecno#1.{\setbox0=\hbox{\toksA={#1.}\toksB={}%
    \def\makenote{\addtokens\toksB{\the\toksC}\def\makenote{\toksD={}
      \toksC={}\let\space\empty}\makenote}\maketoks}}
\def\addtokens#1#2{\edef\addtoks{\noexpand#1={\the#1#2}}\addtoks}
...
\def\makenote{\addtokens\toksB
    {\noexpand\HINTlink{\the\toksC}{\romannumeral\the\toksC}}\toksC={}\global\countC=0}
\def\maketoksdone{\edef\st{\global\noexpand\toksA={\the\toksB}}\st}
...
\def\X#1:#2\X{\ifmmode\gdef\XX{\null$\null}\else\gdef\XX{}\fi %$% section name
  \XX$\langle\,${\let\I=\ne#2\eightrm\kern.5em
    \ifacro{\pdfnote#1.}\else\ifHINT{\HINTnote#1.}\else#1\fi\fi}$\,\rangle$\XX}
\def\Y{\par\yskip}
\let\Z=\le
...
\def\inx{\par\vskip6pt plus 1fil % we are beginning the index
  ...
  \outer\def\I##1, ##2.{\par\hangindent2em\noindent##1:\kern1em
    \ifacro\pdfnote##2.\else\ifHINT\HINTnote##2.\else##2\fi\fi.} % index entry
...
\def\fin{\par\vfill\eject % this is done when we are ending the index
  ...
  \output={\normaloutput\page\lheader\rheader}
  \setpage
  \def\note##1##2.{\quad{\eightrm##1~\ifacro{\pdfnote##2.}\else
    \ifHINT{\HINTnote##2.}\else{##2}\fi\fi.}}
...
% \ifodd\pageno\else\titletrue\null\vfill\eject\fi % for duplex printers
  ...
  \ifHINT\HINThome\fi% Mark the Table of content as home page
