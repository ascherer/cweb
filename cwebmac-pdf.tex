% The following experimental file, by Cesar Augusto Rorato Crusius, can
% be used as an alternative to cwebmac.tex if you use dvipdfm to process
% the TeX output. Please send comments to c.crusius@ieee.org.

% standard macros for CWEB listings (in addition to plain.tex) plus PDF goodies
% Version 3.50 --- December 1999
\ifx\documentstyle\undefined\else\endinput\fi % LaTeX will use other macros
\xdef\fmtversion{\fmtversion+CWEB3.5+PDF}
\newif\ifpdf
% Comment out the following line to restore the normal cwebmac operation
\pdftrue
\def\pdflinkcolor{0 1 1}
...
\def\note#1#2.{\Y\noindent{\hangindent2em%
    \baselineskip10pt\eightrm#1~\ifpdf{\pdfnote#2.}\else#2\fi.\par}}
...
\def\pdflink#1#2{\setbox0\hbox{\special{pdf: bc [ \pdflinkcolor ]}{#1}%
    \special{pdf: ec}}\special{pdf: ann width \thewidth\space height \theheight\space
    depth \thedepth\space << /Type /Annot /Subtype /Link /Border [0 0 0] /A
    << /S /GoTo /D (#2) >> >>}\box0\relax}
...
\def\stsec{\rightskip=0pt % get out of C mode (cf. \B)
  ...
  \ifpdf{\let\*=\empty\special{pdf: dest (\romannumeral\secstar) [ @thispage /FitH @ypos ]}}\fi}
...
\outer\def\N#1#2#3.{\gdepth=#1\gtitle={#3}\MN{#2}% beginning of starred section
  ...
  \ifpdf\special{pdf: outline #1 << /Title (#3) /Dest [ @thispage /FitH @ypos ] >>}\fi
...
\def\X#1:#2\X{\ifmmode\gdef\XX{\null$\null}\else\gdef\XX{}\fi %$% section name
  \XX$\langle\,${\let\I=\ne#2\eightrm\kern.5em\ifpdf{\pdfnote#1.}\else#1\fi}$\,\rangle$\XX}
\def\Y{\par\yskip}
\let\Z=\le
...
\def\inx{\par\vskip6pt plus 1fil % we are beginning the index
  ...
  \outer\def\I##1, {\par\hangindent2em\noindent##1:\kern1em\ifpdf\pdfnote\fi} % index entry
...
\def\fin{\par\vfill\eject % this is done when we are ending the index
  ...
  \output={\normaloutput\page\lheader\rheader}
  \setpage
  \def\note##1##2.{\quad{\eightrm##1~\ifpdf{\pdfnote##2.}\else{##2}\fi.}}
